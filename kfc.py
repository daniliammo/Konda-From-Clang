...
def без_квалификаторов(t: str) -> str:
    ...
    # типа). Убираем только отдельные квалификаторы. Nullability-атрибуты clang
    # («_Nullable»/«_Nonnull»/«_Null_unspecified») — не часть имени типа Konda
    # (их СМЫСЛ учитывает нулевые.py по сырому qualType); из ТИПА убираем, иначе
    # утекли бы в вывод («int _Nullable»).
    t = re.sub(r"\b(const|volatile|restrict"
               r"|_Nullable|_Nonnull|_Null_unspecified)\b", " ", t)
    return " ".join(t.split())
...
class Конвертер:
    def __init__(self, политика=None, исходник_c=None, владение=None, нулевые=None):
        ...
        self.тек_известные_типы = set()  # A-3: имена структур/enum/внешних типов
                                          # для shadow-конфликт-переименования локалей
        ...
        self.буфер_переменные = set()  # указатели, реаллоцируемые в функции → «буфер<T>»
        ...
        self._счётчик = 0
        self.тек_ур = 0             # уровень отступа текущего оператора (для хоста
                                    #   тернарника-в-подвыражении ПЕРЕД этой строкой)
        self.тернарник_нельзя_хостить = False  # True в заголовке цикла: там
                                    #   тернарник вычисляется КАЖДУЮ итерацию, вынос
                                    #   temp ПЕРЕД циклом сломал бы семантику → пометка
    ...
    def _эмит_define_константы(self, декларации):
        """object-like «#define ИМЯ литерал» из ИСХОДНИКА → Konda «конст». Только
        ПРОСТЫЕ литералы (целое/вещественное/символ) и БЕЗ конфликта с именами
        AST — частичное восстановление макро-констант (пункт C). Функциональные
        макросы (с «(» сразу за именем) и сложные значения пропускаем."""
        if not self.исходник_c:
            return
        # Имена, уже занятые декларациями (функции/типы/enum-константы/глобали) —
        # чтобы «конст ИМЯ» не столкнулось с ними (иначе редекларация в Konda).
        занято = set()
        def собрать(d):
        ...
        стр_дефайн = re.compile(r'^\s*#\s*define\s+([A-Za-z_]\w*)([ \t].*)?$')
        эмитнуто = []
        видели = set()
        for стр in self.исходник_c:
            m = стр_дефайн.match(стр)
            if not m:
                continue
            имя, тело = m.group(1), (m.group(2) or "").strip()
            # Функциональный макрос: «(» сразу за именем (без пробела) — не наш.
            конец_имени = m.start(1) + len(имя)
            if конец_имени < len(стр) and стр[конец_имени] == "(":
                continue
            if not тело or имя in занято or имя in видели:
                continue
            видели.add(имя)
            # Отбрасываем хвостовой комментарий значения.
            тело = re.split(r'/\*|//', тело, 1)[0].strip()
            тип_konda = знач = None
            if re.fullmatch(r'-?\d+', тело):
                n = int(тело)
                тип_konda = "целое64" if abs(n) > 2147483647 else "целое32"
                знач = тело
            elif re.fullmatch(r'-?0[xX][0-9a-fA-F]+', тело):
                n = int(тело, 16)                # в десятичное (Konda-лексер)
                тип_konda = "целое64" if abs(n) > 2147483647 else "целое32"
                знач = str(n)
            elif re.fullmatch(r"-?\d+\.\d+[fF]?", тело):
                тип_konda = "вещественное" if тело[-1] in "fF" else "вещественное64"
                знач = тело.rstrip("fF")
            else:
                # строка/символ/выражение/макро-ссылка — не простой числовой
                # литерал (символ-константы как глобаль-конст транспилятор пока
                # не берёт; строки — отдельный тип). Консервативно пропускаем.
                continue
            эмитнуто.append(f"конст {тип_konda} {имя} = {знач}")
        if эмитнуто:
            self._строка("// Восстановленные object-like #define-константы "
                         "(использования уже подставлены литералами):")
            for с in эмитнуто:
                self._строка(с)
            self._строка("")
    ...
    def _натуральная_ширина_иниц(self, n):
        """Ширина ЦЕЛОГО «естественного» типа инициализатора — типа ЗНАЧЕНИЯ до
        неявного сужающего каста (пилим внешние касты/обёртки). None, если не
        целое. Нужна для декл-уровневой проверки сужения, НЕ зависящей от того,
        как конкретная версия clang обернула инициализатор (см. объявление())."""
        while isinstance(n, dict) and n.get("inner") and n.get("kind") in (
                "ImplicitCastExpr", "ConstantExpr", "ParenExpr",
                "ExprWithCleanups", "FullExpr"):
            n = n["inner"][-1]
        if not isinstance(n, dict):
            return None
        return _ширина_целого(qualtype(n))
    ...
    def выражение(self, n) -> str:
        ...
        if k == "ConditionalOperator":
            # Тернарник в ПРОИЗВОЛЬНОЙ под-позиции («f(c?a:b)», «a + (c?b:d)»,
            # «arr[c?i:j]»): в Konda тернарника-выражения нет. Верхние позиции
            # (объявление/return/присваивание) уже разворачивает _тернарник_в_если;
            # здесь — ХОСТ: заводим temp, печатаем «если/иначе» ПЕРЕД текущим
            # оператором (выражение() строится до эмита строки-владельца, поэтому
            # хост-строки ложатся выше — верный порядок), возвращаем имя temp.
            if self.тернарник_нельзя_хостить:
                # Заголовок цикла: temp вычислялся бы один раз до цикла, а не
                # каждую итерацию → семантика бы поехала. Оставляем пометку.
                return (f"/*тернарник — вынесите в если/иначе*/ "
                        f"({self.выражение(вн[0])} ? {self.выражение(вн[1])} "
                        f": {self.выражение(вн[2])})")
            тип = конда_тип(без_квалификаторов(qualtype(n)))
            t = self.врем_имя()
            ур = self.тек_ур
            # temp БЕЗ инициализатора: обе ветви «если/иначе» присваивают →
            # definite-assignment транспилятора удовлетворён (проверено). Так
            # верно и для НЕНУЛЕВОГО указателя (умолчание «нуль» тут отверглось бы).
    ...
    def оператор(self, n, ур):
        ...
        пред_ур = self.тек_ур
        self.тек_ур = ур            # хост тернарника-в-подвыражении печатается
                                    #   на уровне ЭТОГО оператора, строкой ВЫШЕ него
        if n.get("id"):
            self.тек_узел = n["id"]
        try:
    ...
    def _оператор(self, n, ур):
        ...
        if k in ("LabelStmt",):
            ...
            for c in вн:
                self.оператор(c, ур)
            return
        # Вставка ассемблера: «__asm__ volatile (…)» → «асм(…)» в «небезопасно».
        # Синтаксис секций Konda-«асм» совпадает с расширенным asm C 1:1, поэтому
        # берём «(…)»-часть из ИСХОДНИКА (clang JSON не сериализует шаблон/
        # ограничения/порчу — только операнды-выражения; текст надёжнее).
    ...
    def _текст_диапазона(self, n):
        """Исходный текст C, покрытый range узла (для asm: clang не даёт шаблон в
        JSON). Возвращает строку или None. Диапазон asm-кода — ASCII, поэтому
        байтовые колонки clang совпадают с индексами символов."""
        r = n.get("range", {})
        b, e = r.get("begin", {}), r.get("end", {})
        bl, bc = b.get("line"), b.get("col")
        # clang опускает end.line, когда конец на той же строке, что начало
        # (наследование) — тогда end.line == begin.line.
        el = e.get("line") or bl
        ec = e.get("col")
        et = e.get("tokLen", 1)
        ис = self.исходник_c
        if not (bl and bc and el and ec) or bl < 1 or el > len(ис):
            return None
        if bl == el:
            return ис[bl - 1][bc - 1: ec - 1 + et]
        части = [ис[bl - 1][bc - 1:]]
        части.extend(ис[л] for л in range(bl, el - 1))
        части.append(ис[el - 1][: ec - 1 + et])
        return "\n".join(части)

    @staticmethod
    def _разъединить_двоеточия(txt):
        """Пустые секции asm пишутся слитно («"" ::: "memory"»), но лексер Konda
        склеивает «::» в токен области видимости. Разносим последовательности из
        2+ «:» (ВНЕ строк) пробелами → каждый «:» = отдельный ТОКЕН_ДВОЕТОЧИЕ."""
        рез, i, в_строке, эк, кав = [], 0, False, False, ""
        while i < len(txt):
            c = txt[i]
            if в_строке:
                рез.append(c)
                ...
                while j < len(txt) and txt[j] == ":":
                    j += 1
                рез.append(" : ".join([""] * (j - i + 1)) if j - i >= 2 else ":")
                i = j
            else:
                рез.append(c); i += 1
        return "".join(рез)
    ...
    @staticmethod
    def _баланс_скобок(txt, откр):
        """Индекс закрывающей «)», парной к «(» в позиции откр (учёт строк C)."""
        глуб, i, в_строке, эк, кав = 0, откр, False, False, ""
        while i < len(txt):
            c = txt[i]
            if в_строке:
                if эк:            эк = False
    ...
    def _асм(self, n, ур):
        """«__asm__ [volatile] («шаблон» : выходы : входы : порча)» → «асм(…)».
        «(…)»-часть Konda принимает как есть (те же секции, что расширенный asm C).
        Всегда внутри «небезопасно» (сырой доступ к регистрам/памяти)."""
        txt = self._текст_диапазона(n)
        i = txt.find("(") if txt else -1
        j = self._баланс_скобок(txt, i) if i >= 0 else -1
        if txt is None or i < 0 or j < 0:
            # Не удалось извлечь текст (нестандартный range) — честная пометка.
            self.добавить_пометку("асм", n,
                                  деталь="не удалось извлечь текст asm из исходника")
            return
        асм_стр = "асм" + self._разъединить_двоеточия(txt[i:j + 1])
        многострочный = "\n" in асм_стр
        if self.внутри_небезопасно:
            self.эмит(ур, асм_стр)
        elif многострочный:
            self.эмит(ур, "небезопасно {")
            self.эмит(ур + 1, асм_стр)
            self.эмит(ур, "}")
        else:
            self.эмит(ур, "небезопасно { " + асм_стр + " }")
    ...
    def оператор_выражение(self, n, ур):
        ...
        if внр.get("kind") == "CallExpr":
            дети = внр.get("inner", [])
            имя_ф_осв = self.базовое_имя(дети[0]) if дети else ""
            вид_осв = _вид_освобождения(имя_ф_осв)
            if дети and вид_осв and len(дети) == 2:
                ...
                # free(x[…]) — нет. Конвенции «foo_free»/«foo_destroy» — так же,
                # как free (владение из имени: этих знаний в типах C нет).
                ...
                if строгое_имя and строгое_имя in self.срез_переменные:
                    ...
                    if строгое_имя in self.освобождённые_срезы:
                        ...
                        # Деструктор (не чистый free) мог делать больше, чем
                        # освобождение буфера — честно помечаем ПРОВЕРИТЬ.
                        if вид_осв == "деструктор":
                            self.добавить_пометку("деструктор-снят", n,
                                деталь=f"вызов «{имя_ф_осв}({строгое_имя})» снят "
                                       f"(autofree освобождает буфер); если он делал "
                                       f"БОЛЬШЕ, чем free, верните его вручную",
                                ур=ур)
                    return
        ...
        if k_разв == "BinaryOperator" and разв.get("opcode") == "=":
            ...
            # «p = realloc(p, M*sizeof(T))» для растущего буфера → «перевыделить(p, M)».
            # (p попал в буфер_переменные пред-сканом _собрать_буферы.)
            if прав.get("kind") == "CallExpr" and _имя_вызова(прав) == "realloc":
                цель0 = self.развернуть(вн2[0])
                имя_ц = (self.имя_ссылки(цель0)
                         if цель0.get("kind") == "DeclRefExpr" else None)
                арги_r = прав.get("inner", [])[1:]  # [указатель, размер]
                if имя_ц and имя_ц in self.буфер_переменные and len(арги_r) == 2:
                    счёт = self._счёт_из_malloc("malloc", [арги_r[1]])
                    self.эмит(ур, f"перевыделить({self.выражение(вн2[0])}, {счёт})")
                    return
            if прав.get("kind") == "ConditionalOperator":
                # ветви могут быть вложенными тернарниками → рекурсивно. Если ЦЕЛЬ
                # — небезопасный lvalue (напр. «*__errno_location() = …», errno),
                # оборачиваем весь если/иначе в «небезопасно { }» (сырое
                # разыменование за гейтом; иначе транспилятор отвергнет).
                цель_небезоп = (self.небезопасен(вн2[0])
                                and not self.внутри_небезопасно)
                if цель_небезоп:
                    self.эмит(ур, "небезопасно {")
                    прежн = self.внутри_небезопасно
                    self.внутри_небезопасно = True
                    self._присвоить_знач(self.выражение(вн2[0]), вн2[1], ур + 1)
                    self.внутри_небезопасно = прежн
                    self.эмит(ур, "}")
                else:
    ...
    def объявление(self, d, ур, префикс=""):
        ...
        if d.get("kind") != "VarDecl":
            ...
            return
        сырое_имя = d.get("name", "_")
        # A-3: shadow-конфликт локали с ИМЕНЕМ ТИПА («struct buffer *buffer = …»
        # — в C легально, в Konda один неймспейс). Переименовываем через
        # self.переименования → выражение() подхватит DeclRefExpr к этой локали.
        имя = self.переименования.get(сырое_имя, сырое_имя)
        if имя == сырое_имя and сырое_имя in self.тек_известные_типы:
            имя = сырое_имя + "_"
            self.переименования[сырое_имя] = имя
        qt = qualtype(d)
        # Локаль-указатель на функцию «RET (*p)(ARGS) = f»: в Konda тип указателя
        # на функцию — именованный «типфункции». Пред-проход _синтез_фнптр_локалей
        # уже объявил «типфункции RET ИмяN(типы)» на верхнем уровне; здесь эмитим
        # «ИмяN p = f» (присваивание функции по совпадающей сигнатуре).
        разб_фнптр = _разобрать_фнптр(qt)
        if разб_фнптр is not None:
            ...
            if имя_т:
                if дети:
                    ...
                    self.эмит(ур, f"{имя_т} {имя}")
            else:
                # страховка (сигнатура не собрана пред-проходом) — честная пометка
        ...
        if срез is not None:
            ...
            self.эмит(ур, срез)
            return
        # Растущий буфер, объявленный БЕЗ malloc («T* p = NULL» / «T* p;» + realloc
        # позже) → начинаем пустым «буфер<T> p = выделить(0)». malloc-
        # инициализированный буфер уже вернул «буфер<…>» из _malloc_в_срез выше.
        if имя in self.буфер_переменные and kt.endswith("*") and not массив \
                and (иниц is None or нул.это_нуль(иниц)):
            self.срез_переменные.add(имя)
            self.эмит(ур, f"буфер<{kt[:-1]}> {имя} = выделить(0)")
            return
        ...
        if self.небезопасен(иниц):
            ...
            return
        # Структура с ВЛОЖЕННОЙ struct-literal-инициализацией: транспилятор не
        # поддерживает «{ x=1, inner={a=2,b=3} }» напрямую (нестед struct литерал
        # в поле). Разложим на плоские присваивания: «T имя; имя.x=1;
        # имя.inner.a=2; имя.inner.b=3». Работает для не-владеющих структур
        # (у владеющих поле-срез идёт через выделить/клон, что мы не трогаем).
        if not массив and kt in self.поля_структур \
                and иниц.get("kind") == "InitListExpr" \
                and self._есть_вложенный_иниц_список(иниц):
            self.эмит(ур, объ)
            self._декомпоз_иниц_структуры(имя, kt, иниц, ур)
            return
        выр = self.выражение(иниц)
        # Декл-уровневая страховка сужения (size_t/целое64 → целое32 и т.п.):
        # выражение() навешивает «как<>()» по ImplicitCastExpr-узлу, но РАЗНЫЕ
        # версии clang по-разному оборачивают инициализатор (напр. новый clang на
        # CI НЕ дал того ImplicitCastExpr, что старый → «как<>» терялся, и
        # транспилятор ругался «неявное сужение… целое32 ← целое64»). Здесь ловим
        # сужение по СРАВНЕНИЮ ШИРИН объявленного типа и естественного типа
        # значения — устойчиво к форме AST. Не трогаем массивы и уже явный «как<>».
        # kt=="логический": C-конверсия в bool — это «x != 0», НЕ лоссовое сужение
        # (line-559 её тоже не оборачивает: int→bool = IntegralToBoolean, не
        # IntegralCast). Исключаем, иначе навесили бы неверный «как<логический>».
        if not массив and kt != "логический":
            ширина_цели = _ширина_целого(qt)
            ширина_ист = self._натуральная_ширина_иниц(иниц)
            if (ширина_цели is not None and ширина_ист is not None
                    and ширина_цели < ширина_ист            # сужение: цель уже источника
                    and not выр.lstrip().startswith("как<")):
                выр = f"как<{kt}>({выр})"
        self.эмит(ур, f"{объ} = {выр}")

    def _есть_вложенный_иниц_список(self, n):
        if not isinstance(n, dict):
            return False
        for c in n.get("inner", []):
            if isinstance(c, dict) and c.get("kind") == "InitListExpr":
                return True
        return False
    ...
    def _собрать_буферы(self, тело):
        ...
        или «p = realloc(NULL,…)»). Такие становятся «буфер<T>» (растущий буфер),
        а realloc → «перевыделить». Иначе указатель остаётся «срез<T>» (malloc)."""
        буферы = set()
        ...
        def обход(m):
            ...
            if раз.get("kind") == "BinaryOperator" and раз.get("opcode") == "=":
                дети = раз.get("inner", [])
                if len(дети) == 2:
                    цель = self.развернуть(дети[0])
                    прав = self.развернуть(дети[1])
                    if цель.get("kind") == "DeclRefExpr" \
                            and _имя_вызова(прав) == "realloc":
                        имя = (цель.get("referencedDecl") or {}).get("name")
                        if имя:
                            буферы.add(имя)
    ...
    def _malloc_в_срез(self, имя, qt, иниц):
        ...
        if имя_ф not in ("malloc", "calloc") and одиночный:
            return None
        # Указатель реаллоцируется в функции → растущий «буфер<T>» (realloc →
        # «перевыделить»). Индексируется как срез (в срез_переменные добавляется
        # вызывающим). Иначе — обычный владеющий «срез<T>».
        if имя in self.буфер_переменные:
            return f"буфер<{элем}> {имя} = выделить({счёт})"
    ...
    def возврат(self, выр, ур):
        ...
        if внр.get("kind") == "ConditionalOperator":
            ...
            return
        # Небезопасно ли ВЫРАЖЕНИЕ возврата, ИЛИ цикл проверки пометил сам
        # оператор-возврат (тек_узел): транспилятор ругается на строке «вернуть …»,
        # так что политика метит ReturnStmt, а не под-выражение (напр. арифметика
        # «символ» под гейтом §71 — ошибка «небезопасна» указывает на весь возврат).
        # «небезопасен(выр)» видит только выр и его потомков → сам по себе такой
        # случай не поймал бы; добавляем проверку метки оператора.
        помечен = self.тек_узел in self.политика.небезоп_узлы
        if (self.небезопасен(выр) or помечен) and not self.внутри_небезопасно:
    ...
    def цикл_пока(self, n, ур):
        ...
        if self.небезопасен(вн[0]) or self.заблокирован(вн[0]):
            ...
            self.эмит(ур, "}")
            return
        self.тернарник_нельзя_хостить = True   # условие «пока (…)» — каждую итерацию
        усл_с = self._истинность(вн[0])
        self.тернарник_нельзя_хостить = False
        self.эмит(ур, f"пока ({усл_с}) {{")
        self.тело(вн[1], ур)
        self.эмит(ур, "}")
    ...
    def цикл_для(self, n, ур):
        ...
        if доп_инит or len(шаги) > 1:
            for стр in доп_инит:
                self.эмит(ур, стр)
            self.тернарник_нельзя_хостить = True   # заголовок — каждую итерацию
            ...
            self.эмит(ур, "}")
            return
        # Заголовок «для init; cond; inc» вычисляется каждую итерацию (cond/inc) —
        # тернарник тут хостить нельзя (temp встал бы ДО цикла) → пометка.
        self.тернарник_нельзя_хостить = True
        init_s = self._часть(init)
        cond_s = self._истинность(cond) if cond and "kind" in cond else ""
        inc_s = self._часть(inc)
        self.тернарник_нельзя_хостить = False
    ...
    def _обеспечить_известные_типы(self):
        """A-3: множество имён известных типов Konda — struct/union (регистрируются
        в self.поля_структур при регистрации_записи, до эмиссии функций) —
        для детекции shadow-конфликта имени локальной переменной с типом
        («struct buffer *buffer = …» — в C легально, в Konda один неймспейс).
        Перечисления/typedef добавлять не обязательно: shadow с enum-именем
        редок, а typedef-псевдоним указателя на функцию идёт через _разобрать_фнптр.
        В v1 берём только структуры/юнионы — покрывает частый паттерн."""
        # Каждый вызов дозаполняет из свежего self.поля_структур (регистрация
        # записей идёт до функций; в мультифайл-режиме порядок сохраняется).
        for имя in self.поля_структур:
            self.тек_известные_типы.add(имя)
    ...
    def _синтез_фнптр_локалей(self, декларации):
        """Пред-проход: локали-указатели на функцию «RET (*p)(ARGS) = f» → синтез
        «типфункции RET ИмяN(типы)» на ВЕРХНЕМ уровне (в Konda тип указателя на
        функцию именованный). self.фнптр_типы: (возврат, кортеж_пар) → имя типа;
        эмитит typedef'ы (вызывать ДО эмиссии функций). Дедуп по сигнатуре."""
        self.фнптр_типы = {}
        новые = []
        ...
        def сканировать(n):
            ...
            if n.get("kind") == "VarDecl":
                разб = _разобрать_фнптр(qualtype(n))
                if разб is not None:
                    ключ = (разб[0], tuple(разб[1]))
                    if ключ not in self.фнптр_типы:
                        имя_т = f"Фнптр{len(self.фнптр_типы) + 1}"
                        self.фнптр_типы[ключ] = имя_т
                        новые.append((имя_т, разб))
            for c in n.get("inner", []):
                сканировать(c)
        ...
        for d in декларации:
            if d.get("kind") == "FunctionDecl":
                сканировать(d)
        for имя_т, (возврат, пары) in новые:
            self._строка(f"типфункции {возврат} {имя_т}(" + ", ".join(пары) + ")")
        if новые:
            self._строка("")
    ...
    def перечисление(self, d):
        ...
        if not имя:
            ...
            for c in d.get("inner", []):
                ...
                вн = [x for x in c.get("inner", [])
                      if isinstance(x, dict) and "kind" in x and not x.get("kind", "").endswith("Comment")]
                if вн:
        ...
        for c in d.get("inner", []):
            if c.get("kind") != "EnumConstantDecl":
                continue
            вн = [x for x in c.get("inner", []) if isinstance(x, dict) and "kind" in x and not x.get("kind", "").endswith("Comment")]
            if вн:
    ...
    def _анализ_слушателей(self, декларации, все_типы):
        ...
        def распознать(call):
            ...
            if adr.get("kind") != "UnaryOperator" or adr.get("opcode") != "&":
                return
            # Фаза 2 транспилятора (RAII-владение, вариант «а»): userdata слушателя
            # ОБЯЗАН быть владеющим heap-объектом (Ящик<T>) — иначе колбэк, срабатывая
            # позже возврата регистрирующей функции, получит висячий указатель.
            # Heap-идиома: «data» — указатель-переменная (её выделили malloc/xzalloc →
            # kfc даёт Ящик<T>). Адрес стека «&x» под это не подходит → не
            # распознаём идиому, оставляя безопасный сырой fallback (вывод kfc всегда
            # компилируется транспилятором).
            дан = self.развернуть(а_data)
            if дан.get("kind") != "DeclRefExpr":
                return
    ...
    def _читает_имя(self, имя, node):
        """True, если поддерево содержит ЧТЕНИЕ имени (DeclRefExpr к нему)."""
        if not isinstance(node, dict):
            return False
        if node.get("kind") == "DeclRefExpr" and \
                (node.get("referencedDecl") or {}).get("name") == имя:
            return True
        return any(self._читает_имя(имя, c) for c in node.get("inner", []))

    def _первое_исп(self, имя, node):
        """Первое (в порядке документа) использование имени в поддереве:
        'запись' — цель верхнеуровневого «имя = …» (значение НЕ читается справа),
        'чтение' — любое иное упоминание, None — имя не встречено."""
        if not isinstance(node, dict):
            return None
        if node.get("kind") == "BinaryOperator" and node.get("opcode") == "=":
            дети = [c for c in node.get("inner", []) if isinstance(c, dict)]
            if дети and self.базовое_имя(дети[0]) == имя:
                # «имя = имя + 1» — значение всё же читается справа → чтение.
                if len(дети) > 1 and self._читает_имя(имя, дети[1]):
                    return "чтение"
                return "запись"
        if node.get("kind") == "DeclRefExpr" and \
                (node.get("referencedDecl") or {}).get("name") == имя:
        ...
        for c in node.get("inner", []):
            r = self._первое_исп(имя, c)
            if r is not None:
                return r
        return None

    def _первое_исп_в_хвосте(self, имя, хвост):
        """Классификация имени по хвосту cleanup: 'запись' (локаль cleanup-а —
        объявляется ВНУТРИ хелпера), 'чтение' (значение приходит из основной
        функции — параметр хелпера), None."""
        for x in хвост:
            r = self._первое_исп(имя, x)
            if r is not None:
                return r
        return None
    ...
    def _обнаружить_goto_очистку(self, f, тело):
        ...
        исп = []              # параметры хелпера [(имя, полное_объявление)]
        локали_хелпера = []   # локали cleanup-а [(имя, декл-строка)] — объявить
                              # ВНУТРИ хелпера (их значение приходит не извне, а
                              # присваивается в самом хвосте: «errsv = errno»).
        for имя_р in порядок:
            if имя_р in парамы:
                ...
                kt = конда_тип(qualtype(vd))
                нулевой = (kt.endswith("*")
                           and self.нулевые.нулевой(self.тек_исходное_имя, имя_р))
                # Локаль, ПЕРВОЕ использование которой в хвосте — запись
                # («errsv = errno»): значение из основной функции не нужно →
                # объявляем ВНУТРИ хелпера. definite-assignment держится (сначала
                # присвоена, потом читается). Указатель — только nullable (иначе
                # нельзя без инициализатора); срез — владение сложнее, bail.
                if self._первое_исп_в_хвосте(имя_р, хвост) == "запись":
                    if имя_р in self.срез_переменные:
                        return None
                    if kt.endswith("*"):
                        if not нулевой:
                            return None
                        локали_хелпера.append((имя_р, f"возможно<{kt}> {имя_р} = нуль"))
                    else:
                        локали_хелпера.append((имя_р, f"{kt} {имя_р}"))
                    continue
                # Иначе локаль ЧИТАЕТСЯ в хвосте → параметр хелпера.
                if поз >= первый_goto:
                    return None      # локаль объявлена после goto — не определена
                иниц = [x for x in vd.get("inner", [])
                        if isinstance(x, dict) and "kind" in x and not x.get("kind", "").endswith("Comment")]
                if иниц and self._malloc_в_срез(имя_р, qualtype(vd), иниц[-1]) is not None:
                    return None      # локаль-срез — владение сложнее
                if нулевой:
                    # nullable-указатель: конвертер авто-инициализирует «= нуль»
                    # (см. объявление()), значит определён на любом goto → как
                    # параметр «возможно<T*>» (call-site передаёт то же значение).
                    исп.append((имя_р, f"возможно<{kt}> {имя_р}"))
                elif иниц:
                    исп.append((имя_р, f"{kt} {имя_р}"))
                else:
                    return None      # не-nullable без инициализатора — use-before-init
        ...
        return {
            ...
            "локали": локали_хелпера,   # [(имя, декл-строка)] — внутри хелпера
    ...
    def _эмит_goto_хелпер(self, gc):
        ...
        self._строка("{")
        # Локали cleanup-а (первое использование — запись): объявляем внутри
        # хелпера до хвоста. Их значение приходит не из основной функции, а
        # присваивается в самом хвосте (напр. «errsv = errno»).
        for _имя, декл_л in gc.get("локали", []):
            self.эмит(1, декл_л)
        for x in gc["хвост"]:
            self.оператор(x, 1)
    ...
    def функция(self, f):
        ...
        # Растущий буфер: указатель, который где-то в функции РЕАЛЛОЦИРУЕТСЯ
        # («p = realloc(p, …)»), становится «буфер<T>» (не «срез<T>»): realloc →
        ...
        # локалей. Собирается один раз при первой встрече (кэш).
        self._обеспечить_известные_типы()
        ...
        for c in f.get("inner", []):
            ...
            # A-3: shadow-конфликт параметра с ИМЕНЕМ ТИПА (частая C-идиома
            # «void destroy_buffer(struct buffer *buffer)») — переименовываем.
            if сырое_имя not in self.переименования \
                    and сырое_имя in self.тек_известные_типы:
                self.переименования[сырое_имя] = сырое_имя + "_"
...
# Конвенции освобождения памяти по ИМЕНИ функции (владение из исходника, которого
# в типах C нет). Чистые free-алиасы снимаем молча (autofree); деструкторы
# «*_free»/«*_destroy»/… снимаем, но с пометкой ПРОВЕРИТЬ (могли делать больше,
# чем free). Рефкаунт «*_unref»/«*_release» НЕ включаем — освобождает лишь при
# счётчике 0 (снятие было бы неверным).
_FREE_ТОЧНЫЕ = {"free", "g_free", "xfree", "kfree", "vfree"}
_FREE_СУФФИКСЫ = ("_free", "_destroy", "_delete", "_dispose")


def _вид_освобождения(имя):
    """Категория освобождающей функции по конвенции имени: 'free' (чистый алиас),
    'деструктор' (*_free/*_destroy/… — префикс есть), иначе None."""
    if not имя:
        return None
    if имя in _FREE_ТОЧНЫЕ:
        return "free"
    for суф in _FREE_СУФФИКСЫ:
        if имя.endswith(суф) and len(имя) > len(суф):
            return "деструктор"
    return None
...
def сгенерировать(декларации, политика, исходник_c,
    ...
    # NULL-анализ ДО владения: nullable-параметр не должен стать ссылкой.
    ...
    for d in декларации:
        ...
        if k == "RecordDecl" and d.get("completeDefinition"):
            ...
            к.typedef(d)
    # 2а.5: object-like «#define ИМЯ литерал» из ИСХОДНИКА → «конст» (пункт C
    # доклада: clang развернул макросы, в AST их нет; частично ВОССТАНАВЛИВАЕМ
    # сущность-константу из текста. Использования уже подставлены литералами —
    # это документирующая декларация, но макрос как ИМЯ сохранён). Функциональные
    # макросы (ARRAY_SIZE(a)) в Konda не выразимы (§6) — пропускаем.
    к._эмит_define_константы(декларации)
    ...
    # 2б.6: «типфункции» для локалей-указателей на функцию (тела функций).
    к._синтез_фнптр_локалей(декларации)
...
def _поднять_вложенные_аноним_записи(декларации):
    """C-идиома «struct { … } поле;» ВНУТРИ структуры: вложенный анонимный
    RecordDecl теряет имя, поле-ссылка на него имеет qualType «struct (unnamed
    struct at …)», и конда_тип падал в «/*функц-тип*/ ничего» (доступ «n.inner.a»
    не работал).

    Пре-проход именует такие вложенные записи «Внешняя_поле», ПОДНИМАЕТ их на
    верхний уровень как отдельные struct-декларации (Konda не поддерживает
    вложенные записи по типам) и переписывает qualType поля-ссылки на «struct
    ИмяВнешней_поле». Вставка перед внешней структурой — та должна её видеть.

    Позиционная эвристика: сразу за unnamed RecordDecl(complete) в inner
    внешней структуры идёт FieldDecl, ссылающийся на неё. Совпадает с идиомой
    (проверено clang AST); ложных срабатываний нет — unnamed record без сразу
    идущего поля был бы бесполезной декларацией."""
    def обработать(лст):
        i = 0
        новые = []
        while i < len(лст):
            ...
            if isinstance(д, dict) and д.get("kind") == "RecordDecl" \
                    and д.get("completeDefinition") and д.get("name"):
                внутренние = д.get("inner", [])
                j = 0
                while j < len(внутренние):
                    в = внутренние[j]
                    if isinstance(в, dict) and в.get("kind") == "RecordDecl" \
                            and в.get("completeDefinition") and not в.get("name") \
                            and j + 1 < len(внутренние):
                        поле = внутренние[j + 1]
                        if isinstance(поле, dict) and поле.get("kind") == "FieldDecl" \
                                and поле.get("name") \
                                and "(unnamed" in (поле.get("type") or {}).get("qualType", ""):
                            новое_имя = f"{д['name']}_{поле['name']}"
                            в["name"] = новое_имя
                            (поле.setdefault("type", {}))["qualType"] = f"struct {новое_имя}"
                            # Рекурсивно (на случай, если поднятая тоже содержит вложенную)
                            обработать([в])
                            новые.append(в)
                            del внутренние[j]        # RecordDecl уехал наверх
                            continue                  # j теперь указывает на FieldDecl
                    j += 1
            i += 1
        # Вставить поднятые ПЕРЕД декларациями (порядок сохраняем: первый поднятый
        # идёт первым в блоке новых; порядок обработки внешних — сверху вниз).
        for н in reversed(новые):
            лст.insert(0, н)
