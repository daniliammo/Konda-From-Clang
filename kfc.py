...
class Конвертер:
    ...
    def объявление(self, d, ур):
        ...
        объ = f"возможно<{kt}> {имя}" if нулевая else f"{kt} {имя}"
        if массив:
            объ += f"[{разм}]"
        if иниц is None:
...
def собрать_внешние_прототипы(единицы, все_декл, определённые):
    ...
    for имя, список in вызовы.items():
        ...
        for i, п in enumerate(парамы):
            ...
            if not all(адреса) or qt.count("*") != 1 or "[" in qt:
                continue
            баз = qt.replace("*", " ").strip()
            if баз == "char":
                continue                        # char* — строки, не ссылка
            if баз == "void":
                # «void *data»-позиция (главный GUI-паттерн: слушатели/user-data).
                # Типизируем по аргументам: если ВСЕ вызовы дают «&lvalue»
                # ОДНОГО типа T (не char/void, не указатель, не массив) —
                # параметр становится «изменяемый T». C-декларация из заголовка
                # остаётся void*: имплицитная конверсия T* → void* легальна,
                # то же значение адреса уходит в библиотеку.
                типы = set()
                for ад in адреса:
                ...
                if len(типы) != 1:
                    continue                    # разные типы data — оставить сырым
                т_c = типы.pop()
                if not т_c or "*" in т_c or "[" in т_c \
                        or т_c in ("void", "char"):
                    continue
                кт = конда_тип(т_c)
                режимы[i] = ("изменяемый", кт, т_c)
                continue
            режимы[i] = ("чтение" if "const" in qualtype(п) else "изменяемый",
                         None, None)
