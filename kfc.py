...
class Конвертер:
    ...
    def функция(self, f):
        ...
        for c in f.get("inner", []):
            ...
            elif kt.endswith("*") and self.нулевые.нулевой(исходное_имя, pимя):
...
def сгенерировать(декларации, политика, исходник_c,
    ...
    # Заголовки библиотек, чьи функции объявлены «внешняя»: их C-декларации
    # приходят через «#содержит» (сам транспилятор прототип не печатает).
    for заг in (заголовки or []):
        база_заг = os.path.basename(заг)
        if заг.startswith("/usr"):
            к._строка(f"#содержит <{база_заг}>")
        else:
            к._строка(f"#содержит \"{база_заг}\"")
    if заголовки:
        к._строка("")
    # «внешний тип» — до структур (поля-указатели ссылаются на имена);
    # «внешняя»-прототипы — до вызовов (порядок не критичен, но читабельнее).
...
def собрать_внешние_прототипы(единицы, все_декл, определённые):
    ...
    for имя, список in вызовы.items():
        ...
        for i, п in enumerate(парамы):
            ...
            # если «&lvalue» дают ВСЕ вызовы и параметр — одиночный «T*».
        ...
        for i, п in enumerate(парамы):
            ...
            if i in режимы:
                ...
                части.append(f"{режимы[i]} {баз} {имя_п}")
            else:
                имя_т = _имя_польз_типа(qualtype(п))
                if имя_т:
...
def замкнуть_типы(единицы, реф_типы=None):
    ...
    (например, wl_list) требует определения — добавляем его декларацию в единицу
...
def собрать_непрозрачные(единицы, известные, доп_имена):
    ...
    (wl_surface, cairo_t, …) → «внешний тип Имя» (объявление без устройства;
