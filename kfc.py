    ...
    def оператор_выражение(self, n, ур):
        ...
        if k_разв == "BinaryOperator" and разв.get("opcode") == "=":
            ...
            if прав.get("kind") == "ConditionalOperator":
                цель = self.выражение(вн2[0])
                терн = прав.get("inner", [])
                self.эмит(ур, f"если ({self.выражение(терн[0])}) {{")
                self.эмит(ур + 1, f"{цель} = {self.выражение(терн[1])}")
                self.эмит(ур, "} иначе {")
                self.эмит(ур + 1, f"{цель} = {self.выражение(терн[2])}")
                self.эмит(ур, "}")
                return
