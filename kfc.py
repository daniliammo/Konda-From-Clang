...
class Конвертер:
    ...
    def _malloc_в_срез(self, имя, qt, иниц):
        ...
        # их одиночная форма (alloc(sizeof(T))) — владеющий одиночный объект,
        # его цель — Ящик<T>, а не срез<T> длины 1 (тот сломал бы «.поле»).
        # malloc/calloc сохраняют прежнее поведение (в т.ч. одиночная форма),
        # чтобы вывод не менялся там, где он уже принят.
    ...
    def глобальная(self, d, ур=0):
        # Глобал верхнего уровня. В Konda изменяемых глобалов НЕТ (инвариант:
        # гонки невыразимы), но строго ИММУТАБЕЛЬНЫЙ глобал — это «конст»
        # (static const в C). Если объект доказанно неизменяем — печатаем «конст»
        # (иначе транспилятор отвергнет глобал как переменную). Изменяемые
        # остаются без «конст» и дают понятную ошибку (нужен другой механизм).
        префикс = "конст " if d.get("name") in self.неизменяемые_глобали else ""
        self.объявление(d, ур, префикс)
    ...
    def _собрать_неизменяемые_глобали(self, декларации):
        """Имена глобалов, безопасных для «конст»: const-квалифицированные, чей
        объект иммутабелен по типу ЛИБО (указатель-на-const) ни разу не
        переприсваивается в теле функций. Сканируем присваивания/inc/dec."""
        глоб = {d["name"]: qualtype(d) for d in декларации
                if d.get("kind") == "VarDecl" and d.get("name")
                and re.search(r"\bconst\b", qualtype(d))}
        if not глоб:
            return set()
        присвоенные = set()
        def скан(n):
        ...
        for имя, qt in глоб.items():
            if self._тип_глобал_конст(qt) or имя not in присвоенные:
                рез.add(имя)
        return рез
    ...
    def функция(self, f):
        ...
        # «количество_аргументов»/«аргументы» (маппинг на argc/argv в C). C-имена
        # (argc, argv или любые другие) переименовываем и в заголовке, и в теле —
        # иначе обращение к параметру в теле сослалось бы на несуществующее имя.
        канон_вход = ["количество_аргументов", "аргументы"]
        if имя == "точка_входа":
...
def собрать_внешние_прототипы(единицы, все_декл, определённые):
    ...
    for имя, список in вызовы.items():
        ...
        if not годен or (not режимы and not колбэки):
            continue
        части = []
        for i, п in enumerate(парамы):
        ...
        имя_вт = _имя_польз_типа(возврат_qt)
        if имя_вт:
