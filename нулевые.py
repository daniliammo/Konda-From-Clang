...
def проанализировать(декларации, qualtype, без_квалификаторов, политика=None):
    ...
    # Clang nullability-атрибуты «_Nullable»/«_Nonnull» (§ «конвенции/аннотации»):
    # прямое свидетельство владения-nullability из ИСХОДНИКА, надёжнее вывода.
    нуллабл_атр = set()   # (функция, имя) — параметр/локаль помечен «_Nullable»
    ненул_атр = set()     # (функция, имя) — помечен «_Nonnull» → точно НЕ null
    возвр_ненул_атр = set()  # функции с «_Nonnull»-возвратом (не добавлять в возвраты)

    def _атрибут_null(qt, ф, имя):
        # «_Nullable»/«_Nonnull» clang рендерит прямо в qualType («int * _Nullable»).
        if "_Nullable" in qt:
            нуллабл_атр.add((ф, имя))
        elif "_Nonnull" in qt:
            ненул_атр.add((ф, имя))
    ...
    for д in декларации:
        ...
        if тело is None or not имя_ф:
            continue
        тела[имя_ф] = д
        # Возврат функции: «_Nullable» до списка параметров → nullable-возврат;
        # «_Nonnull» → точно не null. Тип возврата — часть qualType ДО первой «(».
        рет_qt = qualtype(д).split("(", 1)[0]
        if "_Nullable" in рет_qt:
            рез.возвраты.add(имя_ф)
        elif "_Nonnull" in рет_qt:
            возвр_ненул_атр.add(имя_ф)
        мест = {}
        for c in д.get("inner", []):
            if c.get("kind") == "ParmVarDecl" and c.get("name") and \
                    _одиночный_указатель(без_квалификаторов(qualtype(c))):
                мест[c["name"]] = True
                _атрибут_null(qualtype(c), имя_ф, c["name"])
        ...
        def локалы(n):
            ...
            if n.get("kind") == "VarDecl" and n.get("name") and \
                ...
                мест[n["name"]] = True
                _атрибут_null(qualtype(n), имя_ф, n["name"])
    ...
    def пометить(ф, имя, сильное):
        if имя not in указатели.get(ф, {}):
            return False
        # «_Nonnull» из исходника — точное свидетельство «не null»: гасит любой
        # вывод nullable (даже сильный) — программист явно гарантировал ненулевой.
        if (ф, имя) in ненул_атр:
            return False
        if сильное:
            сильные.add((ф, имя))
    ...
    # «_Nullable»-атрибуты исходника — сильное свидетельство nullable (до фикспоинта).
    for ф, имя in нуллабл_атр:
        if ф in нулевые:
            пометить(ф, имя, True)
            ...
            def обход(n):
                ...
                if k == "ReturnStmt" and вн:
                    ...
                    if возвр_нул and имя_ф not in рез.возвраты \
                            and имя_ф not in возвр_ненул_атр:
                        рез.возвраты.add(имя_ф)
                        менялось = True
                for c in вн:
                    обход(c)
